...
void rec(chr &res, chr &c1, chr &c2, double R, vector<vector<double> > &mutations, int nv)
{
	...
	//First we need to figure out the relative position of the cross-overs and the mutations
  	for (j = 0; j < nbCo; j++) //generate a position for each cross-over
	{
		k = rnd.rand();
		pos.push_back(k);
	}
	...
	if (pos.size() > 1) //if any cross-overs have occured, record which mutations lie in odd (1) and which lie in even (0) intervals
	{
		...
		if (!(mutations.empty()))
		{
			for (i = 0; i < mutations.size(); i++) //go through mutations
			{
				if (pos.size() > 1)
				{
					for (j = 1; j < pos.size(); j++) //go through cross-over positions
					{
						if(mutations[i][0] > pos[j - 1] && mutations[i][0] < pos[j]) //if a mutation lies between two cross-overs...
							mutations[i][2 + nv] = (j % 2); //record whether it lies in an odd or even interval 
					}
				}
				if(mutations[i][0] > pos.back()) //if position of the mutation is greater than the last cross-over...
					mutations[i][2 + nv] = (pos.size() % 2); //record whether it lies in an odd or even interval
			}
		}
		
		if (!(c1.sel.empty()))
		{
			for (i = 0; i < c1.sel.size(); i++) //include mutations from parent's chromsome 1 (c1) on even intervals between cross-overs
			{
				index = c1.sel[i];
				if (mutations[index][2 + nv] == 0)
		...
		if (!(c2.sel.empty()))
		{
			for (i = 0; i < c2.sel.size(); i++) //include mutations from parent's chromsome 2 (c2) on odd intervals between cross-overs 
			{
				index = c2.sel[i];
				if (mutations[index][2 + nv] == 1)
					res.sel.push_back(index); //add these mutations to offspring's recombined chromosome
			}
		}
	...
	if (pos.size() == 1) //if there are no cross-overs pick one chromosome to pass on at random
	{
		if(rnd.rand() > 0.5) //pick chromosome 1 (c1) to pass on
		{
			if (!(c1.sel.empty()))
			{
				for (i = 0; i < c1.sel.size(); i++) //copy any mutations present on parental chromosome 1 (c1) to offspring (res)
				{
					index = c1.sel[i];
		...
		else //pick chromosome 2 (c2) to pass on
		{
			if (!(c2.sel.empty()))
            {
				for (i = 0; i < c2.sel.size(); i++) //copy any mutations present on parental chromosome 2 (c2) to offspring (res)
				{
					index = c2.sel[i];
					res.sel.push_back(index);
				}
...
void freerec(chr &res, chr &c1, chr &c2)
{
	...
	if (!(hetero.empty()))
	{
		for (i = 0; i < hetero.size(); i++) // loop through heterozygous mutations
		{
